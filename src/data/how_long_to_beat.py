...
def load_prepared_library_data(path):
    ...
    library_hltb['Library Release Year'] = pd.to_datetime(library_hltb['Release Date']).dt.year
...
def filter_and_match_hltb_data(hltb_raw, library_hltb, verbose=True):
    ...
    hltb_filtered = hltb_raw[hltb_raw['similarity'] == hltb_raw.groupby(by='Library ID', as_index=False)['similarity'].transform('max')]
    ...
    hltb_new = hltb_with_library.groupby('Library ID', group_keys=False).apply(select_best_hltb_match)
...
def create_comprehensive_matching_report(hltb_with_library, library_hltb, interm_path):
    ...
    for library_id in hltb_with_library['Library ID'].unique():
        ...
        if max_similarity < 0.75:
            low_similarity_games.append({
                ...
                'HLTB Match': group.loc[group['similarity'].idxmax(), 'game_name'] if 'game_name' in group.columns else 'Unknown'
            })
        ...
        if len(group) > 1:
            ...
            if library_year not in hltb_years:
                year_mismatches.append({
                    ...
                    'Closest HLTB Year': min(hltb_years, key=lambda x: abs(x - library_year))
                })
    ...
    successful_matches = total_library_games - len(no_hltb_records) - len(low_similarity_games)
    
    print("\n" + "="*80)
    print("📈 MATCHING SUMMARY:")
    print(f"   Total library games: {total_library_games}")
    print(f"   Successful matches: {successful_matches} ({successful_matches/total_library_games*100:.1f}%)")
...
def process_hltb_data(config_path='config.yaml', generate_report=True, verbose=True):
    ...
    if generate_report:
        ...
        matching_stats = create_comprehensive_matching_report(hltb_with_library, library_hltb, interm_path)
    ...
    if verbose:
        print(f'HLTB data successfully processed and stored in: {interm_path}hltb_cleaned.csv')
